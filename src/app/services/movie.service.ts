...
export class MovieService {
  ...
  private movies: Movie[] = [
    {
      ...
      description:
        'The grave course of events set in motion by Thanos that wiped out half the universe...',
      price: 200,
      averageRating: 4.2,
      totalRatings: 1250,
      totalReviews: 89,
      ...
      theaters: [
        {
          ...
          amenities: ['IMAX Laser', 'Recliner Seats'],
          rating: 4.8,
        },
      ],
    },
    {
      ...
      theaters: [
        {
          id: 5,
          name: 'Movietime Cinema',
          location: 'Thane',
          showTimes: ['11:30 AM', '3:00 PM', '7:00 PM'],
          amenities: ['4K Projection'],
          rating: 4.1,
        },
      ],
    },
    ...
    },
    {
      ...
      theaters: [
        {
          id: 7,
          name: 'G7 Multiplex (Gaiety Galaxy)',
          location: 'Bandra',
          showTimes: ['12:00 PM', '3:00 PM', '6:00 PM', '9:00 PM'],
          amenities: ['Single Screen'],
          rating: 4.0,
        },
      ],
    },
  ...
  private reviews: MovieReview[] = [
    {
      ...
      rating: 5,
      review:
        'Amazing movie! The action sequences were incredible and the story was well-paced.',
      likes: 12,
  ...
  private bookings: Booking[] = [
    {
      ...
      userId: 1,
      qrCodeUrl:
        'https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=BMS001',
    },
  ];
  ...
  getMovies(): Observable<Movie[]> {
    return this.http.get<any>(`${this.apiUrl}/Movies`).pipe();
  }
  ...
  filterMovies(filters: {
    ...
    if (filters.rating) {
      filtered = filtered.filter(
        (movie) => movie.averageRating >= filters.rating!
      );
    }
  ...
  getMovieReviews(movieId: number): Observable<MovieReview[]> {
    const movieReviews = this.reviews.filter(
  ...
  getMovieComments(movieId: number): Observable<MovieComment[]> {
    const movieComments = this.comments.filter(
  ...
  addRating(
    ...
    const existingReview = this.reviews.find(
      (r) => r.movieId === request.movieId && r.userId === 1
    );
    ...
    return of({ success: true, message: 'Rating added successfully' }).pipe(
      delay(500)
    );
  }
  ...
  addComment(
    ...
    return of({ success: true, message: 'Comment added successfully' }).pipe(
      delay(500)
    );
  }
  ...
  cancelBooking(
    ...
    return of({ success: false, message: 'Booking not found' }).pipe(
      delay(500)
    );
  }
  ...
  addMovie(movie: Omit<Movie, 'id'>): Observable<Movie> {
    const newMovie: any = {
      id: Math.max(...this.movies.map((m) => m.movieId)) + 1,
      ...movie,
    };
  ...
  updateMovie(movieToUpdate: Movie): Observable<{ success: boolean; movie?: Movie }> {
  return this.http.put<void>(`${this.apiUrl}/Movies/${movieToUpdate.movieId}`, movieToUpdate).pipe(
    map(() => ({ success: true, movie: movieToUpdate })),
    catchError((error) => {
  ...
  deleteMovie(movieId: number): Observable<{ success: boolean }> {
    console.log(movieId)
    return this.http.delete<void>(`${this.apiUrl}/Movies/${movieId}`).pipe(
      map(() => ({ success: true })), // success if no error and status is 204
      catchError((error) => {
